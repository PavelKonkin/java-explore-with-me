...
public class AdminCompilationServiceTest {
    ...
    @InjectMocks
    private AdminCompilationServiceImpl compilationService;
    ...
    private EventCompilation eventCompilation3;
    ...
    public void setup() {
        ...
        updatedCompilation = compilation.toBuilder()
                ...
                .build();
        eventCompilation3 = EventCompilation.builder()
                .event(event3)
    ...
    public void add_whenSuccessful_thenReturnCompilationDto() {
        ...
        when(eventCompilationRepository.saveAll(List.of(eventCompilation1, eventCompilation2)))
                .thenReturn(List.of(eventCompilation1, eventCompilation2));
        when(compilationMapper.convertCompilation(compilation)).thenReturn(compilationDto);
        ...
        verify(eventCompilationRepository, times(1)).saveAll(any());
    ...
    public void add_whenCompilationWithoutEvents_thenReturnCompilationDtoWithoutEvents() {
        ...
        when(compilationMapper.convertCompilation(compilationWithoutEvents)).thenReturn(compilationDtoWithoutEvents);
        ...
        verify(eventCompilationRepository, never()).saveAll(any());
        verify(compilationMapper, times(1)).convertCompilation(any(Compilation.class));
    }
    ...
    public void patch_whenSuccessful_thenReturnUpdatedCompilationDto() {
        ...
        doNothing().when(eventCompilationRepository).deleteAllByCompilationId(compilation.getId());
        when(eventRepository.findAllByIdIn(eventIds)).thenReturn(List.of(event3));
        when(eventUtilService.getHitsByEvent(eventIds))
                .thenReturn(hitsByEvent);
        when(eventUtilService.getConfirmedRequestCountById(eventIds))
                .thenReturn(confirmedRequestCount);
        when(eventMapper.convertEventToShortDto(event3)).thenReturn(eventShortDto3);
        when(eventCompilationRepository.saveAll(List.of(eventCompilation3)))
                .thenReturn(List.of(eventCompilation3));
        when(compilationMapper.convertCompilation(updatedCompilation)).thenReturn(updatedCompilationDto);
        ...
        verify(eventCompilationRepository, times(1))
                .deleteAllByCompilationId(compilation.getId());
        verify(eventRepository, times(1)).findAllByIdIn(eventIds);
        verify(eventUtilService, times(1))
                .getHitsByEvent(eventIds);
        verify(eventUtilService, times(1))
    ...
    public void patch_whenCompilationNotFound_thenThrownException() {
        ...
        verify(eventCompilationRepository, never())
                .deleteAllByCompilationId(anyLong());
        verify(eventRepository, never()).findAllByIdIn(any());
        verify(eventUtilService, never())
